...
// НОВЫЙ МАРШРУТ: Отображение Политики конфиденциальности (ЗАГЛУШКА)
app.get('/privacy-policy', (req, res) => {
    // В реальном приложении здесь должен быть отправлен файл (e.g., public/privacy_policy.html)
    // или отрендерен шаблон.
    res.send(`
        <!DOCTYPE html>
        <html lang="ru">
        <head>
            <meta charset="UTF-8"><title>Политика конфиденциальности</title>
            <style>
        ...
        <body>
            ...
            <p><strong>Дата вступления в силу:</strong> 3 декабря 2025 г.</p>
            
            <p>Данный документ описывает, как ваш сервис собирает, использует и защищает информацию, 
            которую вы предоставляете при использовании нашего сайта.</p>

            <h2>1. Сбор и использование информации</h2>
            <p>Мы собираем информацию, которую вы предоставляете нам напрямую, включая ваше имя, 
            адрес электронной почты, пароль (в захешированном виде), информацию о городе, стране, 
            контактном телефоне и предпочтениях по активности.</p>

            <h2>2. Согласие</h2>
            <p>Нажимая кнопку регистрации и устанавливая соответствующий флажок, вы подтверждаете, 
            что прочитали, поняли и согласны с нашей Политикой конфиденциальности.</p>
            
            <p><em>Пожалуйста, замените этот текст на полный, юридически корректный текст вашей политики.</em></p>

            <button onclick="window.close()" style="margin-top: 20px; padding: 10px 20px; cursor: pointer;">Закрыть</button>
        </body>
        </html>
    `);
});
...
app.post("/register", async (req, res) => {
    try {
        // ✅ НОВОЕ: Извлекаем также поле 'consent'
        const { name, email, password, consent } = req.body; 

        // 🛑 ПРОВЕРКА СОГЛАСИЯ
        if (!consent) {
             // Возврат к /register.html, чтобы пользователь мог прочитать и согласиться
            return res.status(400).send(`
                <h2>Ошибка Регистрации</h2>
                <p>Вы должны согласиться с <a href="/privacy-policy" target="_blank">Политикой конфиденциальности</a> для продолжения.</p>
        ...
        const newUser = { 
            ...
            activities: [],
            // ✅ НОВОЕ: Сохраняем факт согласия с датой
