    ...
    router.post('/send-message', async (req, res) => { 
        try {
            ...
            await db.collection('messages').insertOne({ 
                ...
                imageId: imageId || null, source: source || "Галерея",
                text: messageText, reply: null, createdAt: new Date(), isRead: false
            }); 
            res.json({ status: 'ok' });
        } catch (error) { res.status(500).json({ error: 'Ошибка отправки' }); }
    });
    ...
    router.get("/login", async (req, res) => {
        try {
            ...
            let tasksHtml = `<div class="gallery-grid">` + pageData.tasks.map(t => {
                ...
                return `<div class="gallery-wrapper" onclick="openModal('${t._id}', '${t.userId}', '${url}', '${t.originalName}')"><div class="gallery-item work-border">${content}</div>${statusHtml}</div>`;
            }).join('') + `</div>`;
            ...
            res.send(` 
                <!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><title>Вход</title><script src="/ga.js"></script><style>
                    ...
                    .gallery-grid { display: flex; flex-wrap: gap: 10px; justify-content: flex-start; }
                    ...
                    .work-border { border: 2px solid orange; } .ready-border { border: 2px solid #28a745; }
                    ...
                    <div id="photoModal" class="modal-overlay"><div class="modal"><span class="close-modal" onclick="closeModal()">&times;</span><h3 id="modalTitle" style="margin-top:0; color:black;">Фото</h3><img id="modalImg" src=""><div id="actionButtons" class="modal-buttons"><a id="viewLink" href="#" target="_blank" class="btn-view">👁️ Просто посмотреть</a><button onclick="showChatForm()" class="btn-chat">💬 Написать сообщение</button></div><div id="msg-form"><label style="color:black; font-weight:bold;">Ваш контакт:</label><input type="text" id="contactInfo" placeholder="Email или телефон..."><label style="color:black; font-weight:bold;">Сообщение:</label><textarea id="messageText" placeholder="Привет..."></textarea><button onclick="sendMessage()" style="background:#007BFF">Отправить</button></div></div></div>

                    <div class="page-section">
                        <div class="main-wrapper">
                            <div class="block">
                                <h3>Вход</h3><form action="/login" method="POST"><input type="hidden" name="_csrf" value="${res.locals.csrfToken}"><input type="email" name="email" placeholder="Email" required><input type="password" name="password" placeholder="Пароль" required><button type="submit">Войти</button><a href="/register" class="link">Нет аккаунта? Регистрация</a></form><hr><h3>Активности:</h3><a href="/activities/Шахматы" target="_blank" class="activity-btn chess-btn">♟️ Шахматы (${pageData.chessCount})</a><a href="/activities/Футбол" target="_blank" class="activity-btn foot-btn">⚽ Футбол (${pageData.footballCount})</a><a href="/activities/Танцы" target="_blank" class="activity-btn dance-btn">💃 Танцы (${pageData.danceCount})</a>
                            </div>
                    ...
                    <script>
                        let currentToUserId = '', currentImageId = '';
                        function openModal(id, userId, url, title) { document.getElementById('photoModal').style.display = 'flex'; document.getElementById('modalImg').src = url; document.getElementById('modalTitle').innerText = title; document.getElementById('viewLink').href = url; document.getElementById('actionButtons').style.display = 'flex'; document.getElementById('msg-form').style.display = 'none'; document.getElementById('messageText').value = ''; currentToUserId = userId; currentImageId = id; }
                        ...
                        async function sendMessage() { const text = document.getElementById('messageText').value; const contact = document.getElementById('contactInfo').value; if(!text) return alert('Напишите сообщение!'); const res = await fetch('/send-message', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-csrf-token': "${res.locals.csrfToken}" }, body: JSON.stringify({ toUserId: currentToUserId, imageId: currentImageId, messageText: text, contactInfo: contact, source: 'Галерея' }) }); if(res.ok) { alert('Отправлено!'); closeModal(); } else { alert('Ошибка'); } }
